...
) -> Recipe | None:  # Récupère une recette par son ID. Si la recette n'existe pas, retourne None.
...
def get_or_create_ingredient(db: Session, ingredient_name: str) -> Ingredient:
    db_ingredient = (
        db.query(Ingredient).filter(Ingredient.name == ingredient_name.strip().lower()).first()
    )
